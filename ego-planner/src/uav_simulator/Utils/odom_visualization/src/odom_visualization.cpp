...
void cmd_callback(const geometry_msgs::PoseStamped cmd)
{   
  if (cmd.header.frame_id == string("null"))
    return;
  
  colvec pose(6);
  pose(0) = cmd.pose.position.x;
  pose(1) = cmd.pose.position.y;
  pose(2) = cmd.pose.position.z;
  colvec q(4);
  q(0)    = 1.0;
  q(1)    = 0.0;
  q(2)    = 0.0;
  ...
  meshROS.type = visualization_msgs::Marker::MESH_RESOURCE;
  meshROS.action = visualization_msgs::Marker::ADD;
  meshROS.pose.position.x = cmd.pose.position.x;
  meshROS.pose.position.y = cmd.pose.position.y;
  meshROS.pose.position.z = cmd.pose.position.z;
...
int main(int argc, char** argv)
{
  ...
  n.param("prefix", prefix, string("ardrone1"));   
